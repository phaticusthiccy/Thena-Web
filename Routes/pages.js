...
var random = require("./main/plugins/random")
...
point.get('/showcase', async (req, res) => {
    return res.sendFile(__dirname + "/main/showcase.html")
})

point.get('/showcase_img', async (req, res) => {

    var temp_array = random(
        fs.readdirSync(__dirname + "/main/src/showcase/")
        .filter(
            (obj) => obj.includes("Thena")
        )
    )
